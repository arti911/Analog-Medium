<template>
  <section class="hero is-fullheight section">
    <div class="section">
      <div class="container">
        <h1 class="title" v-if="posts.length === 0">Постов {{ countPost }}</h1>
        <div class="cards">
          <div class="card" v-for="(post, index) in pagePosts[current - 1]" :key="post.id">
            ...
            <div class="card-footer">
              ...
              <button class="like" @click="likePost(index, post.id)" v-if="role === 'reader' || role === null" :class="!role ? 'disabled' : null">
                <span>
              ...
              <div class="card-footer-item" v-if="role === 'writer'">
                <div class="buttons">
...
export default {
  ...
  computed: {
    ...
    countPost () {
      return this.$store.getters.countPost
    }
  },
  methods: {
    getPosts () {
    ...
    likePost (index, id) {
      ...
      this.$store.dispatch('LIKE_POST', this.$store.state.posts[index])
    },
    validDate (date) {
